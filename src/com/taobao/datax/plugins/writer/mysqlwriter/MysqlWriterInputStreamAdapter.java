...
public class MysqlWriterInputStreamAdapter extends InputStream {
	...
	public int read(byte[] buff, int off, int len) throws IOException {
		...
		while (len > 0) {
			// if (lineCounter % 10 == 0) {
			// System.out.println("***************oracle**write*****len***" + lineCounter + " /" + len);
			// }
	...
	private void buildString(Line line) {
		...
		String field;
		for (int i = 0, num = line.getFieldNum(); i < num; i++) {
	...
	private int fetchLine(byte[] buff, int off, int len) throws UnsupportedEncodingException {
		...
		/*
		 * 该buffer已经不够放一个line，因此把line的内容保存下来，供下丄1�7次fetch使用 这里的假设是previous足够处1�7 绝对够容纳一个line的内宄1�7
		 */
		/* fix bug: */
